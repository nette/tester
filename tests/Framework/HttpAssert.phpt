...
// Starts a local HTTP server (see fixtures/http-server.php). The built-in server needs
// the CLI SAPI, so under php-cgi/phpdbg the sibling php(.exe) binary is used, if any.
$cli = PHP_SAPI === 'cli'
	? PHP_BINARY
	: preg_replace('~php-cgi(\.exe)?$~i', 'php$1', PHP_BINARY);
if (($cli === PHP_BINARY && PHP_SAPI !== 'cli') || !is_file($cli)) {
	Tester\Environment::skip('Requires the PHP CLI binary to run the built-in web server.');
}
...
$proc = proc_open(
	[$cli, '-n', '-S', "127.0.0.1:$port", __DIR__ . '/fixtures/http-server.php'],
