...
test('attribute selectors', function () {
	...
	Assert::same("div[substring(@foo, string-length(@foo) - string-length('bar') + 1)='bar']", DomQuery::css2xpath('div[foo$="bar"]'));
	Assert::same("div[@foo='bar[]']", DomQuery::css2xpath("div[foo='bar[]']"));
	Assert::same("div[@foo='bar[]']", DomQuery::css2xpath('div[foo="bar[]"]'));
});


test('attribute values with quotes', function () {
	Assert::same("div[@foo=\"it's\"]", DomQuery::css2xpath('div[foo="it\'s"]'));
	Assert::same("div[@foo='\"bar\"']", DomQuery::css2xpath("div[foo='\"bar\"']"));
	Assert::same("div[@foo=concat('a', \"'\", 'b\"c')]", DomQuery::css2xpath("div[foo=a'b\"c]"));
});
